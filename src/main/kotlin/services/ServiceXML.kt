...
class ServiceXML {
    ...
    fun writeXMLResiduos(residuosDTO: List<ResiduoDTO>) {
        val ficheroResiduos = File(directorio + File.separator + "output" + File.separator + "modelo_residuos_2021.xml")
    ...
    fun writeXMLContendores(contenedorDTO: List<ContenedorDTO>) {
        val ficheroContenedor = File(directorio + File.separator + "output" + File.separator + "contenedores_varios.xml")
        val xml = XML {indent = 4}
