...
class StorageCSV {
     ...
     *
     * @param directorio Es la ruta contenedora del fichero CSV o bien propio fichero. Este método se ocupará de buscar en directario, el fichero que necesitamos o, si se trata de un fichero en si mismo trabajar con él.
     *
    ...
    fun readResiduo(directorio: String): List<ResiduoDTO> {
        ...
        if (!directorio.endsWith(".csv")) {
            ficheroResiduo = File(directorio + File.separator + "modelo_residuos_2021.csv")
        }
        ...
        if (ficheroResiduo.exists() && validateFileExtension(ficheroResiduo.toString())) {
            if (longitudCabeceraResiduos(
                    ...
                    ficheroResiduo.readLines().first().split(";")
                )
            ) {
    ...
    fun readContenedor(directorio: String): List<ContenedorDTO> {
        logger.info("Leyendo CSV.")
        var ficheroContenedor = File(directorio)
        if (!directorio.endsWith(".csv")) {
            ficheroContenedor = File(directorio + File.separator + "contenedores_varios.csv")
        }

        if (ficheroContenedor.exists() && validateFileExtension(ficheroContenedor.toString())) {
            if (longitudCabeceraContenedores(ficheroContenedor.readLines().first()) && separacionColumnas(
                    ficheroContenedor.readLines().first()
                ).equals(ficheroContenedor.readLines().first().split(";"))
            ) {
