...
class ServiceJSON {
    ...
    fun writeJSONResiduo(residuoDTO: List<ResiduoDTO>) {
        val ficheroResiduo = File(directorio + File.separator + "modelo_residuos_2021.json")
        val json = Json { prettyPrint = true }
        ficheroResiduo.writeText(json.encodeToString(residuoDTO))
    }

    fun writeJSONContenedor(contenedorDTO: List<ContenedorDTO>) {
        val ficheroContenedor = File(directorio + File.separator + "contenedores_varios.json")
        val json = Json { prettyPrint = true }
